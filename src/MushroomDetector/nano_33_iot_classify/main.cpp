...
void loop()
{
  ...
  char OldLabel[15];
  char NewLabel[15];
  char result_label[15];
  char result_value[15];
  ...
  for (int i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++)
  {
    if (strcmp(result.classification[i].label, "AmbientAir") != 0 && result.classification[i].value > max_val)
    {
      max_val = result.classification[i].value;
      max_idx = i;
    }
  }
  // Show only max value of mushroom
  if (max_val == 0.000)
  {
    sprintf(result_label, "%s", "NoMushroom");
    sprintf(result_value, "%.1f", 0.000);
  }
  else
  {
    sprintf(result_label, "%s", result.classification[max_idx].label);
    sprintf(str_buf, "%.3f", max_val);
    float x = (atof(str_buf) * 100);
    sprintf(result_value, "%.1f", x);
  }
  ...
  if (result.anomaly < ANOMALY_THRESHOLD)
  {
    ...
    sprintf(NewLabel, "%s", result_label);
  ...
  ei_printf("t: %.2f, h: %.2f, b: %.2f, g: %.2f", temp.value(), hum.value(), baro.value(), gas.value());
  ei_printf("\r\n");
